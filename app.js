...
function renderAdminEmps(dept, btnElement) {
   ...
   if (window.currentEmpSubTab === 'sellers') {
       ...
       Object.keys(deptGroups).forEach(deptName => {
           ...
           // Заголовок отдела продавцов
           sellersHtml += `<div class="grid-details-title" style="color:gray; font-size:13px; font-weight:bold; margin-top:14px; margin-bottom:8px; text-transform:none; border-bottom:1px solid var(--border-color); padding-bottom:4px; text-align:left;">${deptName}:</div>`;
