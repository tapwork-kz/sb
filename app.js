import { setKpiColor, buildStandardRow, generateHorizontalGrid, groupAndRenderByMonth, generateDatePanelHTML } from './ui.js';
import { saveMemory, getMemory, clearMemory, safeIin, fmtSum, formatNumberWithSpaces, formatPointsNoun, formatShortName, formatDateLocal, isCurrentMonth, getMonthName, parseCustomDate, requestNotificationPermission, showPushNotification, showToast, vibrate, getSourceColor, formatRemarkAuthor, formatRemarkText } from './utils.js';
      ...
      if (promoLists.length > 0) {
          ...
          promoLists.forEach((list, lIdx) => {
              ...
              list.items.forEach((item, iIdx) => {
                  ...
                  // ПЕРЕХВАТ ССЫЛКИ СТРОГО ВО ВНУТРЕННИЙ БРАУЗЕР (Защита от редиректа)
                  let linkBtn = link ? `<div onclick="event.stopPropagation(); event.preventDefault(); if(window.Telegram && window.Telegram.WebApp && window.Telegram.WebApp.openLink) { window.Telegram.WebApp.openLink('${link}'); } else { window.open('${link}', '_blank', 'noopener,noreferrer'); }" style="display:flex; align-items:center; justify-content:center; background:#f39c12; color:white; width:22px; height:22px; border-radius:5px; margin-right:8px; flex-shrink:0; box-sizing:border-box; border:1px solid rgba(0,0,0,0.05); cursor:pointer;"><span class="material-symbols-rounded" style="font-size:16px;">open_in_new</span></div>` : '';
...
function openScDoc() { 
    if (selectedScItem && selectedScItem.docUrl) { 
        ...
        else window.open(selectedScItem.docUrl, '_blank', 'noopener,noreferrer'); 
    } 
}

async function executeSubmit(type, details, targetIin = null, meta = "", customMsg = null) { vibrate(50); showToast("Отправка...", false, 9999); let res = await callBackend('submitRequest', { type: type, details: details, targetIin: targetIin, metadata: meta }); if(res.success) { showToast(customMsg || "Запрос успешно отправлен!"); closeForm(); loadDashboard(true); } else showToast("Ошибка: " + res.error, true); }
...
async function processReq(id, action, replyText = "") { vibrate(50); showToast("Обработка...", false, 9999); processedReqIds.add(String(id)); let el = document.getElementById("req-" + id); if (el) { el.style.display = 'none'; } let res = await callBackend('processRequest', { reqId: id, reqAction: action, replyText: replyText }); if(res.success) { showToast(res.msg); loadDashboard(true); } else { showToast(res.error, true); loadDashboard(true); } }
...
window.submitPromoCheck = function(typeText, valText, ptsText, lIdx, iIdx, prefixType) {
    ...
    let exec = () => {
        ...
        if (cntEl) {
            ...
            if (cur > 1) { cntEl.innerText = cur - 1; } 
            else { document.getElementById(`promo-item-${lIdx}-${iIdx}`).style.display = 'none'; }
        }
        executeSubmit(prefixType, typeText, null, metaStr);
    };
    
    if (typeof tg !== 'undefined' && tg && tg.showPopup) {
        try { tg.showPopup({ title: prefixType, message: promptMsg, buttons: [{id: 'yes', type: 'ok', text: 'Да'}, {type: 'cancel', text: 'Отмена'}] }, function(btnId) { if (btnId === 'yes') exec(); }); } 
        catch(e) { if (confirm(promptMsg)) exec(); }
    } else { if (confirm(promptMsg)) exec(); }
};
...
// ПРОБРОС ФУНКЦИЙ В ГЛОБАЛЬНЫЙ КОНТЕКСТ ДЛЯ HTML
window.NOM_DICT = NOM_DICT;
window.appState = appState;
window.calcPlanEngine = calcPlanEngine
