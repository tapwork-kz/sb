...
function renderPlanUI(pData) {
    ...
    html += `<div class="inner-block card" style="margin-bottom:12px; padding:12px; background:var(--card-bg); border:1px solid var(--border-color);">
        ...
        <div style="background:var(--card-bg); border-radius:12px; padding:10px; margin-bottom:8px; border:1px solid var(--border-color);">
            ...
            <div style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap:4px; text-align:center; align-items:start;">
                <div><div style="color:gray; font-size:9px; margin-bottom:2px;">ПЛАН</div><div style="color:var(--text-color); font-size:13px; font-weight:normal; letter-spacing:-0.5px;">${fmtSum(pData.to.total.plan)}</div></div>
        ...
        <div style="background:var(--card-bg); border-radius:12px; padding:10px; margin-bottom:8px; border:1px solid var(--border-color);">
            ...
            <div style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap:4px; text-align:center; align-items:start;">
                <div><div style="color:gray; font-size:9px; margin-bottom:2px;">ПЛАН</div><div style="color:var(--text-color); font-size:13px; font-weight:normal; letter-spacing:-0.5px;">${fmtSum(pData.aks.total.plan)}</div><div style="color:gray; font-size:9px; font-weight:bold; margin-top:4px;">Цель: <span style="color:var(--text-color);">${pData.aks.total.targetPct}%</span></div></div>
    ...
    let buildRow = (dTo, dAks, dUsl, isFact) => {
        ...
        let fw = isFact ? 'normal' : 'normal'; 
        return `
            <div style="color:gray; font-size:9px; text-align:left;">${lbl}</div>
            <div style="color:${cTo}; font-size:12px; font-weight:${fw};">${fmtSum(fTo)}</div>
            <div style="color:${cTo}; font-size:12px; font-weight:${fw};">${fmtSum(fAks)}</div>
            <div style="color:${cTo}; font-size:12px; font-weight:${fw};">${fmtSum(fUsl)}</div>`;
    };
    for (let i = 0; i < 3; i++) {
...
function generateDatePanelHTML(idPrefix, onChangeFuncName) {
    ...
    <div class="inner-block card date-panel-wrapper" style="padding:12px; margin-bottom:12px; background:var(--card-bg); border:1px solid var(--border-color);">
        <div class="no-swipe" style="display:flex; gap:6px; align-items:center;" ontouchstart="event.stopPropagation();" ontouchmove="event.stopPropagation();">
            <input type="date" id="${idPrefix}-start" value="${defStart}" style="flex:1; background:var(--card-bg); border:1px solid var(--border-color); color:var(--text-color); border-radius:8px; padding:0; line-height:34px; font-size:12px; text-align:center; margin:0; height:36px; box-sizing:border-box; letter-spacing:-0.5px;">
            <span style="color:gray; font-weight:bold;">-</span>
            <input type="date" id="${idPrefix}-end" value="${defEnd}" style="flex:1; background:var(--card-bg); border:1px solid var(--border-color); color:var(--text-color); border-radius:8px; padding:0; line-height:34px; font-size:12px; text-align:center; margin:0; height:36px; box-sizing:border-box; letter-spacing:-0.5px;">
...
async function callBackend(actionName, payloadData = {}) { 
  try { 
    ...
    if (actionName === "startupCheck") {
      ...
      const todayStart = new Date(); todayStart.setHours(0,0,0,0); const currentHour = new Date().getHours();
      ...
      return { 
          ...
          activeOuts: Object.values(activeOutsMap).map(o => {
              ...
              if (rRole.includes('промоутер')) {
                  if (o.action === 'Обед') timerLimit = 60; else if (o.action === 'Полдник') timerLimit = 30; else timerLimit = 15;
              } else {
                  if (o.action === 'Обед') timerLimit = 40; else if (o.action === 'Полдник') timerLimit = 30; else timerLimit = 10;
              }
    ...
    pollingTimer = setInterval(async () => { 
        ...
        if(appState.token && !document.hidden) { 
            ...
            if(state) { 
                ...
                if (appState.role.toLowerCase().includes("директор") || appState.role.toLowerCase().includes("заведующий")) renderAdminOuts(); 
                else applyLimits(state); 
            }
...
async function triggerAutoReturn(actionToReturnFrom) {
    ...
    await callBackend('recordAction', { token: appState.token, iin: appState.iin, actionType: actionToReturnFrom, isReturn: true, isAutoReturn: true }); 
    let state = await callBackend('startupCheck', { token: appState.token, iin: appState.iin }); 
    applyLimits(state);
}
...
async function triggerReturn() { 
    vibrate(50); const actionToReturnFrom = appState.currentAction; 
    appState.currentAction = null; saveMemory("currentAction", ""); renderTimeUI(); 
    document.querySelectorAll("#standard-buttons button").forEach(b => b.disabled = true); 
    document.getElementById("btn-break").disabled = false; document.getElementById("action-hint").innerText = "Фиксируем возвращение..."; 
    ...
    if (res.success) { 
        ...
        appState.currentAction = actionToReturnFrom; saveMemory("currentAction", actionToReturnFrom); renderTimeUI(); 
        showToast("Ошибка возврата: " + res.error, true); 
        let state = await callBackend('startupCheck', { token: appState.token, iin: appState.iin }); applyLimits(state); 
    } 
}
  ...
  if (type === 'sc') { 
      ...
      window.triggerMyScReload = function(t, val) {
          if(t && t !== 'search') setPanelDates(t, val, 'my-sc', () => window.triggerMyScReload('search'));
              ...
              let h = "";
              if(arr.length > 0) {
                  h += arr.map((i, idx) => buildStandardRow({title: `${idx + 1}. ${i.reason}`, typeText: i.source, typeColor: getSourceColor(i.source), dateText: i.date, hasBorder: false})).join("");
              } else { h = "<div style='padding:15px;text-align:center;color:gray;font-size:13px;'>В выбранном периоде пусто</div>"; }
              document.getElementById('my-sc-list-container').innerHTML = h;
          }
      };
      window.triggerMyScReload('search');
      return; // Прерываем функцию, так как HTML уже вставлен
  } 
  else if (type === 'points') { 
...
function openEmpScDetails(iin) {
    ...
    <div class="inner-block card date-panel-wrapper" style="padding:12px; margin-bottom:12px; background:var(--card-bg); border:1px solid var(--border-color);">
        <div class="no-swipe" style="display:flex; gap:6px; align-items:center;" ontouchstart="event.stopPropagation();" ontouchmove="event.stopPropagation();">
            ...
            <div style="position:relative; width:36px; height:36px; flex-shrink:0;">
                <input type="month" onchange="setEmpScDates('month', this.value, '${iin}'); this.value='';" style="position:absolute; top:0; left:0; width:100%; height:100%; opacity:0; cursor:pointer;">
                <button class="btn-gray" style="margin:0; width:100%; height:100%; border-radius:8px; padding:0; display:flex; justify-content:center; align-items:center; background:var(--card-bg); border: 1px solid var(--border-color); color:var(--text-color); font-size:16px;">🗓️</button>
            </div>
