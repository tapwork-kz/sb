...
function getSourceColor(src) { 
    ...
    if(s.includes('сц')) return '#e67e22'; if(s.includes('trade-in')) return '#8e44ad'; if(s.includes('горячий')) return '#e84393'; if(s.includes('обмен')) return '#f39c12'; if(s.includes('исправл')) return '#3498db'; if(s.includes('мотивац')) return '#3390ec'; 
...
/* =========================================
   5. ПОЛЬЗОВАТЕЛЬСКИЙ ИНТЕРФЕЙС И РЕНДЕР
   ========================================= */
...
function renderDashboardData(data, isSilent = false) {
  ...
  if (uHistList) {
      ...
      uHistList.innerHTML = groupAndRenderByMonth(filteredUHistory, r => {
          ...
          let titleColor = getSourceColor(metaObj.type || r.type); 
  ...
  if(adminList) {
      adminList.innerHTML = aInbox.map(r => { 
          ...
          let authorStr = r.type === "Замечание" ? `<b style="color:#f39c12;">${formatRemarkAuthor(r.authorName, r.authorRole)}</b>` : `<b>От:</b> ${r.adminDisplayName || r.authorName || ''}`; let finalDescHtml = r.type === "Замечание" ? desc + selDateHtml : `<b>Детали:</b> ${desc}${selDateHtml}`; 
          let titleColor = getSourceColor(metaObj.type || r.type); 
...
function renderAdminScItems(dept, btnElement) {
   ...
   if (currentAdminScTabType === 'active') { 
       ...
       container.innerHTML = groupAndRenderByMonth(sold, r => { 
           ...
           const tagColor = getSourceColor(metaObj.type || r.type); 
           let displayAct = r.actUrl || metaObj.docUrl || ""; let displayDisc = r.discount || metaObj.discount || "0%"; let rawDetails = String(r.details || ""); let match = rawDetails.match(/\n\[(.*?)\]$/); let approverName = ""; if (match) { approverName = formatShortName(match[1]); rawDetails = rawDetails.replace(/\n\[(.*?)\]$/, "").trim(); } let actHtml = displayAct ? `📄 <a href="${displayAct}" target="_blank" style="color:#3390ec; text-decoration:none; font-weight:bold;" onclick="event.stopPropagation()">Акт товара</a>` : (isFocus ? '' : '<span style="color:gray; font-size:10px;">(Акт не прикреплен)</span>'); let approverHtml = approverName ? `<div style="margin-top:6px; font-size:10px; color:gray; text-align:right;">Одобрил: ${approverName}</div>` : ''; 
           return `<div class="inner-block sc-item card" onclick="this.classList.toggle('selected')" style="padding:10px; margin-bottom:8px; border-left: 3px solid ${tagColor}; cursor: pointer; background: var(--card-bg);"><div style="display:flex; justify-content:space-between; margin-bottom:4px;"><span class="type-label" style="font-size:9px; font-weight:bold; color:${tagColor};">${metaObj.type || 'СЦ'}</span><span style="font-size:9px; color:gray;">${r.date}</span></div><div style="font-size:12px; font-weight:bold; margin-bottom:4px;">${rawDetails}</div><div style="font-size:11px; line-height:1.4;">👤 <span style="color:gray;">Продавец:</span> <b>${r.authorName}</b><br>${displayDisc !== "0%" ? `🏷️ <span style="color:gray;">Скидка:</span> <b style="color:#e74c3c;">${displayDisc}</b><br>` : ''}${actHtml}</div>${approverHtml}</div>`; 
       });
   }
}
...
/* =========================================
   6. ОБРАБОТЧИКИ ДЕЙСТВИЙ И КНОПОК
   ========================================= */
window.submitPromoCheck = function(typeText, valText, ptsText, lIdx, iIdx, prefixType) {
    ...
    let d = new Date(); let formattedDate = ("0" + d.getDate()).slice(-2) + "." + ("0" + (d.getMonth() + 1)).slice(-2) + "." + d.getFullYear();
    ...
    let exec = () => {
        ...
        if (cntEl) { let cur = parseInt(cntEl.innerText) || 0; if (cur > 1) { cntEl.innerText = cur - 1; } else { document.getElementById(`promo-item-${lIdx}-${iIdx}`).style.display = 'none'; } }
        executeSubmit(prefixType, typeText, null, metaStr);
    };
    if (typeof tg !== 'undefined' && tg && tg.showPopup) { try { tg.showPopup({ title: prefixType, message: promptMsg, buttons: [{id: 'yes', type: 'ok', text: 'Да'}, {type: 'cancel', text: 'Отмена'}] }, function(btnId) { if (btnId === 'yes') exec(); }); } catch(e) { if (confirm(promptMsg)) exec(); } } else { if (confirm(promptMsg)) exec(); }
};
...
/* =========================================
   7. УПРАВЛЕНИЕ UI (Вкладки, Модалки, Формы)
   ========================================= */
...
/* =========================================
   8. СЛУШАТЕЛИ СОБЫТИЙ И ЗАПУСК (INIT)
   ========================================= */
