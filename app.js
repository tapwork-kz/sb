...
function renderDashboardData(data, isSilent = false) {
  ...
  let countSc = monthSc.filter(p => p && (p.source === "СЦ" || p.source === "СЦ/Дефект")).length; 
  let countFocus = monthSc.filter(p => p && (p.source !== "СЦ" && p.source !== "СЦ/Дефект")).length; 
...
function renderAdminEmps(dept, btnElement) {
   ...
   container.innerHTML = filtered.map(e => { 
           ...
           let curMonthSc = monthScHist.filter(p => p.source === "СЦ" || p.source === "СЦ/Дефект").length; 
           let curMonthFocus = monthScHist.filter(p => p.source !== "СЦ" && p.source !== "СЦ/Дефект").length; 
